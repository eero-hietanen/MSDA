...
plotting_server <- function(id, data) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$plot_output <- renderPlotly({
      ...
      ggplotly(data$p) %>% highlight(on = 'plotly_selected', off = 'plotly_deselect') %>% config(scrollZoom = TRUE, modeBarButtonsToRemove = c('zoomIn', 'zoomOut', 'hoverCompareCartesian', 'hoverClosestCartesian')) %>% style(unselected=list(marker=list(opacity=1)))
    })
