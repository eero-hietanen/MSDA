...
proteinGroups <- fread("proteinGroups.txt")
annotation.mq <- fread("annotation.csv")
...
# Convert MaxQuant data to MSstatsTMT input. Preview the input through
# head(input) to check that everything looks right.

tidymess("Building MsStats input from MaxQuant files.")

input <- MaxQtoMSstatsTMTFormat(evidence, proteinGroups, annotation.mq)
